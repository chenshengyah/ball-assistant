...
- `编辑图文详情`：`isManageable = true`、`status != CANCELLED`、活动未结束
...
- `GENERAL`：直接对活动报名，不选择场地，后续分场不在系统内处理
...
- `GENERAL / 统一分配` 不提供用户报名后再挂到具体场地的状态迁移
...
- 活动已取消：`当前活动已取消，不能再编辑图文详情`
- 活动已结束：`活动已结束，不能再编辑图文详情`
- 无权限：`你没有管理该活动的权限`
...
- 若原活动引用场地已被删除，仍允许进入再次发布，但必须提示并重新确认
...
| ---------- | ---------------- | ------------------ | -------------- |
| 草稿 | 无 | 无 | 继续编辑 / 编辑图文详情 |
| 报名中 | 报名 | 取消报名 | 编辑图文详情 / 取消活动 / 调场 / 调容量 / 再次发布 |
| 报名截止 | 无 | 无 | 编辑图文详情 / 取消活动 / 调场 / 调容量 / 再次发布 |
| 进行中 | 无 | 无 | 编辑图文详情 / 调场 / 调容量 / 再次发布 |
| 已结束 | 再次查看 | 再次查看 | 再次发布 |
| 已取消 | 再次查看 | 再次查看 | 再次发布 |
